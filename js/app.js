...
function itemInstructions(item) {
  if (item.type === ItemType.ContactInformation) {
    return "Enter information to make a QR Code. A device that scans the QR Code will add the information into its address book.";
  } else if (item.type == ItemType.WebLink) {
    return "Enter information to make a QR Code. A device that scans the QR Code will open the web link in its browser.";
  } else if (item.type === ItemType.CallPrompt) {
    return "Enter information to make a QR Code. A device that scans the QR Code will be prompted to call the number.";
  } else if (item.type === ItemType.MessagePrompt) {
    return "Enter information to make a QR Code. A device that scans the QR Code will be prompted to send a message to the number.";
  } else if (item.type === ItemType.EmailPrompt) {
    return "Enter information to make a QR Code. A device that scans the QR Code will be prompted to send an email to the address.";
  }
}
